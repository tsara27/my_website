    ...
    return (
      ...
      >
        <Link href={`/blog/${post.slug}`}>
          <h3 className="font-display text-gray text-2xl cursor-pointer xs:mb-4 sm:mb-6 md:mb-10 tracking-tighter xs:mt-8 sm:mt-14 md:mt-16 hover:text-dove-gray">
            {post.title}
          </h3>
        </Link>
        <p className="font-regular font-light text-lg text-dove-gray sm:pb-5 md:h-66 lg:h-64 md:mt-6">{post.excerpt}</p>
        <div className="border-b border-light-gray xs:hidden sm:block mt-6"></div>
        <Link
          href={`/blog/${post.slug}`}
